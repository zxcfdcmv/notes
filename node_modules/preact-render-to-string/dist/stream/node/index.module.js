      ...
      if (!asyncMode && renderer && renderer.onError) {
        const onError = error => {
          return renderer.onError(error, vnode, (child, parent) => {
            try {
              return _renderToString(child, context, isSvgMode, selectValue, parent, asyncMode, renderer);
            } catch (e) {
              return onError(e);
            }
          });
...
async function renderToChunks(vnode, {
  ...
  const shell = renderToString(vnode, context, renderer); // Wait for any suspended sub-trees if there are any
  ...
  if (len > 0) {
    ...
    const initialWrite = docSuffixIndex !== -1 ? shell.slice(0, docSuffixIndex) : shell;
