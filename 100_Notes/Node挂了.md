...
## 对集群调度与资源的压力
- **新节点资源紧张/Pending：** 大量被驱逐的 Pod 集中在其他节点重建，会导致其他节点的 CPU 和内存使用率瞬间飙升。如果集群整体资源不足，部分 Pod 将会卡在 **`Pending`** 状态无法启动。
- **DaemonSet 的特殊表现：** 运行在该节点上的守护进程（如日志收集 Fluentd、网络插件 Calico）会随着节点一起失去响应，但 K8s 绝不会将 DaemonSet 类型的 Pod 调度到其他节点上去代替它。
