# 核心思想
**为每个容器分配一个可路由的IP地址，构建一个扁平(不封装)的三层网络**, 让Pod之间直接通过IP路由通信，无需NAT，无需隧道，像物理服务器一样
...
- **非扁平（Overlay网络）**：当使用VXLAN/IPIP时，数据包的原始Pod IP会被“藏”在一个外层包头里，物理网络只看到外层物理机的IP。这就像在原有道路上空架了一层高架桥，数据包需要“上桥”和“下桥”，这就是“叠加”的概念。
## 二层/三层网络
